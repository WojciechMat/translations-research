...
from translations.data.management import EuroparlDataManager, TranslationDataset
...
def create_tmx_files(data_manager: EuroparlDataManager, 
                    output_dir: str,
                    src_lang: str, 
                    tgt_lang: str,
                    train_ratio: float = 0.9,
                    use_val_for_tuning: bool = True) -> Tuple[str, str]:
    """
    ...
    if use_val_for_tuning:
        ...
        tune_dataset = TranslationDataset(
            source=train_dataset.source[split_idx:],
            reference=train_dataset.reference[split_idx:]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset-path", default="translations/hf_datasets/EuroparlEnPl", 
                        help="Path to Europarl dataset")
    parser.add_argument("--cache-dir", default="./tmp/data/cache", help="Cache directory")
    parser.add_argument("--random-seed", type=int, default=42, help="Random seed")
    parser.add_argument("--use-val-for-tuning", action="store_true", 
                        help="Use validation set for tuning (otherwise split training set)")
    parser.add_argument("--train-ratio", type=float, default=0.9, 
                        help="Ratio of training data if splitting the train set")
